* General coding stuff

** Modes

#+BEGIN_SRC emacs-lisp

  ;; (use-package protobuf-mode)
  (use-package json-mode
    :preface
    (defun roland/json-sort-keys ()
      "Sort JSON keys using jq. Silently skips if jq is not found or JSON is invalid."
      (let* ((output-buffer (generate-new-buffer " *json-sort*"))
             (exit-code (call-process-region (point-min) (point-max)
                                             "jq" nil output-buffer nil
                                             "--sort-keys" ".")))
        (if (= exit-code 0)
            (let ((pos (point)))
              (replace-buffer-contents output-buffer)
              (goto-char (min pos (point-max))))
          (message "json-sort: skipped (jq error or invalid JSON)"))
        (kill-buffer output-buffer)))
    :hook
    (json-mode . (lambda ()
                   (add-hook 'before-save-hook #'roland/json-sort-keys nil t))))
  (use-package yaml-mode)
  (use-package markdown-mode
    :config
    (setq markdown-command "/home/yoda/.asdf/shims/marked"))
  (use-package markdown-toc)
  (use-package markdown-preview-mode
    :config
    (setq markdown-preview-stylesheets
          (list "https://cdn.jsdelivr.net/npm/github-markdown-css/github-markdown.css"
                "data:text/css,body{box-sizing:border-box;min-width:200px;max-width:980px;margin:0 auto;padding:45px;}"))
    (setq markdown-preview-javascript
          (list "https://cdn.jsdelivr.net/npm/mermaid/dist/mermaid.min.js"))
    (setq markdown-preview-script-onupdate
          "document.body.className='markdown-body';
           document.querySelectorAll('code.language-mermaid').forEach(function(el) {
             var div = document.createElement('div');
             div.className = 'mermaid';
             div.textContent = el.textContent;
             el.parentNode.replaceWith(div);
           });
           mermaid.run();"))
  ;; (use-package edit-indirect)
  ;; (use-package makefile-executor
  ;;   :config
  ;;   (add-hook 'makefile-mode-hook 'makefile-executor-mode)
  ;;   )

#+END_SRC

** Flycheck

Enables flycheck. On the fly syntax checking (e.g. use =M-x
flycheck-list-errors= to see what's wrong in the current file).

#+BEGIN_SRC emacs-lisp

  (use-package flycheck
    :init
    ;; (global-flycheck-mode)
    :config
    (setq flycheck-disabled-checkers '(emacs-lisp-checkdoc))
    (setq flycheck-markdown-markdownlint-cli-config ".markdownlint.json")
    )

#+END_SRC

** Company

Enables company. At cursor auto-completion (with pop-up). 

#+BEGIN_SRC emacs-lisp

  (use-package company
    :defer 0.1
    :config
    (global-company-mode t)
    (setq-default company-idle-delay 0.1)
    (setq-default company-require-match nil)
    (setq-default company-minimum-prefix-length 3)

    ;; get only preview
    ;; (setq-default company-frontends '(company-preview-frontend))
    ;; also get a drop down
    (setq-default company-frontends '(company-pseudo-tooltip-frontend company-preview-frontend))
    )

#+END_SRC

** YasSnippet

Enables (code) snippets.

*Not* using this for now.

#+BEGIN_SRC emacs-lisp

  ;; (use-package yasnippet
  ;;   :bind (
  ;;          ("C-c C-y e" . 'yas-expand)
  ;;          )
  ;;   :config
  ;;   (yas-global-mode)
  ;;   )

  ;; (use-package yasnippet-snippets)

#+END_SRC

** YaFolding

Folding code (based on indention).

*Not* using this for now.

#+BEGIN_SRC emacs-lisp

  ;; (use-package yafolding
  ;;   :bind (
  ;;          ("C-c C-y t a" . 'yafolding-toggle-all)
  ;;          ("C-c C-y t e" . 'yafolding-toggle-element)
  ;;          )
  ;;   :hook (
  ;;          (prog-mode . yafolding-mode)
  ;;          )
  ;;   )

#+END_SRC

** Projectile

Enable projectile project management.

*Not* using this for now.

#+BEGIN_SRC emacs-lisp

  ;; (use-package projectile
  ;;   :config
  ;;   (add-to-list 'projectile-globally-ignored-directories "_build")
  ;;   (add-to-list 'projectile-globally-ignored-directories "deps")
  ;;   (add-to-list 'projectile-globally-ignored-directories ".elixir_ls")
  ;;   (add-to-list 'projectile-globally-ignored-directories "target")
  ;;   (add-to-list 'projectile-globally-ignored-directories ".bloop")
  ;;   (add-to-list 'projectile-globally-ignored-directories ".bsp")
  ;;   (add-to-list 'projectile-globally-ignored-directories ".metals")

  ;;   (setq projectile-project-search-path '("~/Development/Work/" "~/Development/Home/"))
  ;;   (define-key projectile-mode-map (kbd "C-c C-p") 'projectile-command-map)
  ;;   :init
  ;;   (projectile-mode)
  ;;   )

#+END_SRC

** Magit

Make git/magit work.

#+BEGIN_SRC emacs-lisp

  (use-package magit
    :bind (
           ("C-z m s" . 'magit-status)
           )
    :config
    (setq magit-pull-or-fetch t)
    (define-key magit-mode-map "f" 'magit-pull)
    (define-key magit-mode-map "F" nil)
    :custom
    ;; (magit-wip-mode t)
    (magit-no-confirm (quote (safe-with-wip)))
    (magit-define-global-key-bindings nil)
    ;; (magit-display-buffer-noselect t)
    ;; Options are:
    ;;   magit-display-buffer-traditional,
    ;;   magit-display-buffer-same-window-except-diff-v1,
    ;;   magit-display-buffer-fullframe-status-v1,
    ;;   magit-display-buffer-fullframe-status-topleft-v1,
    ;;   magit-display-buffer-fullcolumn-most-v1,
    ;;   display-buffer,
    ;;   ignore
    (magit-display-buffer-function 'magit-display-buffer-fullframe-status-v1)
    )

#+END_SRC

** Forge

*Note:* (Obviously) For forge to work you need to put the
=~/.authinfo= or =~/.netrc= file in place.

#+BEGIN_SRC emacs-lisp

  (use-package forge
    :after magit
  )

  ;; copy the url to the PR into the CnP buffer (when/after creating a PR) 
  (defun my-forge-post-submit-callback-kill-url (value _headers _status _req)
    (when t
      (when-let ((url (alist-get 'html_url value)))
        (kill-new url))))

  (add-hook 'forge-post-submit-callback-hook 'my-forge-post-submit-callback-kill-url)

#+END_SRC

** Code Review

*Not* using this for now (because of the warnings, while loading).

#+BEGIN_SRC emacs-lisp

  ;; (use-package code-review
  ;;   :bind (
  ;;          :map prog-mode-map
  ;;          ("C-z r f" . 'code-review-forge-pr-at-point)
  ;;          ("C-z r s" . 'code-review-submit-comments)
  ;;          )
  ;;   :after forge
  ;;   :config
  ;;   (setq code-review-auth-login-marker 'forge)
  ;;   (setq code-review-fill-column 80)
  ;;   (add-hook 'code-review-mode-hook #'emojify-mode)
  ;; )

#+END_SRC

** (Better) Terminal/Shell

*Note:* You need to =sudo apt install libtool-bin libvterm-dev=.

#+BEGIN_SRC emacs-lisp

  (use-package vterm
    :custom-face
    ;; ANSI color 0 (black) defaults to near-invisible on solarized-light background.
    ;; Use base01 (#586e75) for foreground and base00 (#657b83) for bright variant.
    (vterm-color-black ((t (:foreground "#586e75" :background "#657b83"))))
  )

#+END_SRC

** Editor Config

#+BEGIN_SRC emacs-lisp

  (use-package editorconfig
    :ensure t
    :config
    (editorconfig-mode 1)
  )

#+END_SRC

